...
export const LoginPage: React.FC<LoginPageProps> = ({ staffProfiles, onLogin, onStartOnboarding }) => {
  const [email, setEmail] = useState('');
  const [accessCode, setAccessCode] = useState('');
  const [error, setError] = useState('');

  const admin = staffProfiles.find((p) => p.role === 'admin');
  const sampleStaff = staffProfiles.find((p) => p.role === 'staff');
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-6 items-stretch">
        ...
        <section className="bg-white rounded-3xl p-6 md:p-8 shadow-2xl">
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 space-y-3">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Seeded test accounts</p>
              ...
              {admin && (
                <button
                  ...
                  onClick={() => {
                    ...
                    setAccessCode('admin2026');
                  }}
                  className="w-full text-left p-3 bg-slate-50 hover:bg-slate-100 rounded-xl border border-slate-100"
                >
                  <strong>Admin:</strong> {admin.email} <span className="text-slate-400">/ admin2026</span>
                </button>
              )}
              {sampleStaff && (
                <button
                  type="button"
                  onClick={() => {
                    setEmail(sampleStaff.email);
                    setAccessCode('staff2026');
                  }}
                  className="w-full text-left p-3 bg-slate-50 hover:bg-slate-100 rounded-xl border border-slate-100"
                >
                  <strong>Staff:</strong> {sampleStaff.email} <span className="text-slate-400">/ staff2026</span>
                </button>
              )}
            </div>
